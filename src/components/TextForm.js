    ...
    <div className={`maincontainer mx-4 py-4 my-5 text-${props.mode==="light"?"dark":"light"} hello py-3 my-5`} id='borderme' style={{border:'2px solid black'}} >
        <div className="container py-3 hello" >
          <h2 className='py-2'>{props.heading}</h2>
              ...
              <button className="btn btn-primary mx-1 my-1"  onClick={onhandleUp}>Convert to uppercase</button>
              <button className="btn btn-primary mx-1 my-1"  onClick={onhandlelow}>Convert to lowercase</button>
              {/* <button className="btn btn-primary mx-1 my-1"  onClick={toggleMode1}>{btnText}</button> */}
              <button className="btn btn-primary mx-1 my-1"  onClick={ExtraSpaceremoval}>Remove extra spaces</button>
              <button className="btn btn-primary mx-1 my-1"  onClick={copyText}>Copy text to clipboard</button>
              <button className="btn btn-primary mx-1 my-1"  onClick={onhandleclear}>Clear</button>
        </div>
        <div className="container my-2 py-2 px-20 hello" >
        <h4>Summary of the text you entered</h4>
        <p>Words typed: {text.length>0?text.split(" ").length:0}   and  Letters typed: {text.length-text.split(" ").length+1}</p>
        <p>Average time to be taken to read the text you entered is {0.008*text.length} mins</p>
